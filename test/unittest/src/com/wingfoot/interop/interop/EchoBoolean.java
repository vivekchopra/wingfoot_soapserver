...
public class EchoBoolean  implements InteropInterface
{
  ...
  **/
	  ...
	  //  requestEnvelope.setSchemaInstance(schemaInstance);
	    ...
	    call.addParameter("inputBoolean", b, Class.forName("java.lang.Boolean"));
	    call.setMethodName("echoBoolean");
	    ...
	    Envelope responseEnvelope = call.invoke(transport);
		  ...
		  return "Fault: " + f.getFaultString();
	              ...
	              Boolean theReturnData=null;

                      if (responseEnvelope.getParameter(0) instanceof String) {
                          String s = (String) responseEnvelope.getParameter(0);
			  if (s.equals("1"))
			      theReturnData = new Boolean("true");
			  else if (s.equals("0"))
			      theReturnData = new Boolean("false");
                          else 
			      theReturnData = new Boolean(s);

                      }
		      else 
		            theReturnData = (Boolean) 
                                 responseEnvelope.getParameter(0);;
		      if ( ! theReturnData.booleanValue() )
